...
namespace Latihan_6_1
{
    public partial class frmCustomer : Form
    {
        ...
        void reset()
        {
            command = new MySqlCommand("select ifnull(max(id),0)+1 from customer", koneksi);
        ...
        void showAll()
        {
            command = new MySqlCommand("select * from customer", koneksi);
            oDA = new MySqlDataAdapter(command);
            oDT = new DataTable();
            oDA.Fill(oDT);
        ...
        void filteredSelect()
        {
            command = new MySqlCommand("select * from customer where kode like concat('%', @kode, '%') and nama like concat('%', @nama, '%')", koneksi);
        ...
        void insertData()
        {
            ...
            {
                koneksi.Open();
                command = new MySqlCommand("insert into customer values(@id,@kode,@nama,@alamat,@createdat,@editedat)", koneksi);
        ...
        void updateData()
        {
            ...
            try
            {
                command = new MySqlCommand("update customer set kode=@kode,nama=@nama,Alamat=@alamat,created_at=@createdat,updated_at=@editedat where id=@id", koneksi);
                command.Parameters.AddWithValue("@id", txtID.Text);
                command.Parameters.AddWithValue("@kode", txtKode.Text);
                command.Parameters.AddWithValue("@nama", txtNama.Text);
                command.Parameters.AddWithValue("@alamat", txtJumlahAwal.Text);
